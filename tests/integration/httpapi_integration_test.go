...
func TestHTTPAPITwitchCallbackViewerConsumesState(t *testing.T) {
	...
	api := handlers.New(handlers.Dependencies{
		...
		ViewerOAuth: func(_ context.Context, code string, gotPayload core.OAuthStatePayload, lang string) (string, string, error) {
			...
			if gotPayload.TelegramUserID != payload.TelegramUserID || gotPayload.Mode != core.OAuthModeViewer {
				t.Fatalf("ViewerOAuth() payload = %+v, want TelegramUserID=%d, Mode=%q", gotPayload, payload.TelegramUserID, core.OAuthModeViewer)
			}
			if lang != "it" {
				t.Fatalf("ViewerOAuth() lang = %q, want \"it\"", lang)
			}
			return "success", "viewer-name", nil
		},
		CreatorOAuth: func(_ context.Context, _ string, _ core.OAuthStatePayload, _ string) (string, string, error) {
